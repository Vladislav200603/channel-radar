...
- простий сигнал аномалії: пост має щонайменше `2×` медіани останніх відомих переглядів
  усіх зібраних постів за вибраний період;
...
помилку, час останньої спроби й останнього успішного збору. Прогрес історії розділений:
активний cursor і черга для пропусків між недавніми зборами зберігаються окремо від
cursor початкового архіву. Тому повільне дозавантаження старих постів не блокує свіжі.
...
- Telegram web-preview не надає стабільної структурованої ознаки для всіх типів
  пересланих повідомлень, тому репости поки зберігаються як звичайні пости каналу;
- UI позначає дані як `stale` після трьох годин без успішного збору. GitHub Actions може
  затримати запланований запуск, тому на безкоштовному хостингу цей статус іноді означає
  затримку scheduler, а не поломку джерела.

## AI-шар

Окрема дія на сторінці каналу надсилає до Gemini до 40 останніх постів за сім днів.
Цей горизонт навмисно фіксований і не залежить від періоду, вибраного для графіків:
дайджест відповідає окремому продуктовому питанню «про що канал писав останній тиждень».
...
Поточний набір містить **128 офлайн-тестів**, включно з upgrade/downgrade міграцій,
видаленням каналу, scheduler, пагінацією, захистом AI-відповідей і збором архіву.
